...
int janus_websockets_init(janus_transport_callbacks *callback, const char *config_path) {
	...
	JANUS_LOG(LOG_VERB, "kimi___ wsc->count_threads %d\n", lws_get_count_threads(wsc));
...
//	JANUS_LOG(LOG_VERB, "kimi___ janus_websockets_destroy_client+++\n");	// kimi			
	if(!ws_client || !ws_client->ts) {
//		JANUS_LOG(LOG_VERB, "kimi___ janus_websockets_destroy_client--- !ws_client || !ws_client->ts\n");	// kimi	
		return;
	}

//	JANUS_LOG(LOG_VERB, "kimi___ janus_websockets_destroy_client lock+++\n");	// kimi
...
//		JANUS_LOG(LOG_VERB, "kimi___ janus_websockets_destroy_client---\n");	// kimi	
	...
	/* Cleanup */
//	JANUS_LOG(LOG_VERB, "kimi___ janus_websockets_destroy_client Destroying WebSocket Client\n");	// kimi
...
//		JANUS_LOG(LOG_VERB, "kimi___ janus_websockets_destroy_client gateway->notify_event\n");	// kimi
	...
	janus_mutex_unlock(&ws_client->ts->mutex);
	/* Notify core */
//	JANUS_LOG(LOG_VERB, "kimi___ janus_websockets_destroy_client gateway->transport_gone\n");	// kimi	
...
//		JANUS_LOG(LOG_VERB, "kimi___ janus_websockets_thread lws_service+++\n");		// kimi
		lws_service(service, 50);
//		JANUS_LOG(LOG_VERB, "kimi___ janus_websockets_thread lws_service--- ret %d\n", nret);		// kimi
	}
...
//				JANUS_LOG(LOG_VERB, "kimi___ LWS_CALLBACK_SERVER_WRITEABLE lock+++\n");	// kimi
				janus_mutex_lock(&ws_client->ts->mutex);
//				JANUS_LOG(LOG_VERB, "kimi___ LWS_CALLBACK_SERVER_WRITEABLE lock---\n");	// kimi
...
//					JANUS_LOG(LOG_VERB, "kimi___ LWS_CALLBACK_SERVER_WRITEABLE lws_write complete pending write\n");		// kimi
...
//					JANUS_LOG(LOG_VERB, "kimi___ LWS_CALLBACK_SERVER_WRITEABLE lws_callback_on_writable\n");		// kimi
					lws_callback_on_writable(wsi);
					janus_mutex_unlock(&ws_client->ts->mutex);
//					JANUS_LOG(LOG_VERB, "kimi___ LWS_CALLBACK_SERVER_WRITEABLE unlock---\n");	// kimi
					return 0;
				}
...
//					JANUS_LOG(LOG_VERB, "kimi___ LWS_CALLBACK_SERVER_WRITEABLE lws_write sending message\n");		// kimi
...
/*	
	{	// kimi... add for test
		JANUS_LOG(LOG_VERB, "kimi__ janus_websockets_common_callback +++ %s %d\n", janus_websockets_reason_string(reason), reason);		// kimi
		int nret = janus_websockets_common_callback(wsi, reason, user, in, len, FALSE);
		JANUS_LOG(LOG_VERB, "kimi__ janus_websockets_common_callback --- %s %d\n", janus_websockets_reason_string(reason), reason);		// kimi
		return nret;
	}
*/
