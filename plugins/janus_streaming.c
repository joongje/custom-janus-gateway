...
#include <unistd.h>				// add kimi
#include <sys/syscall.h>		// add kimi	
#include <sys/types.h>			// add kimi
...
static gboolean _bRecvCheckTS = FALSE;	// add kimi
static uint32_t _recvSaveTS;			// add kimi
...
typedef struct janus_streaming_rtp_relay_packet {
	...
	uint32_t origin_timestamp;		// add kimi for process duration check
	uint16_t origin_seq_number;		// add kimi for process duration check
	gint64   queuing_nowUs; 		// add kimi for process duration check
...
/////////////////////////////////////////////////////
// add kimi REMB ABR state enum
typedef enum simulcast_abr_state {
	_REMB_BITRATE_HOLD_STATE = 0,
	_REMB_BITRATE_INCREASE_STATE = 1,
...
typedef struct simulcast_abr_state_context {
	...
	simulcast_abr_state _prev_state;
	uint64_t			_remb_bitrate;
	uint64_t			_prev_remb_bitrate;
	uint16_t			_hold_state_count;
	uint16_t			_stream_changed_waiting_count;
	uint16_t			_slow_start_overcome_count;
} simulcast_abr_state_context;
/////////////////////////////////////////////////////
...
typedef struct janus_streaming_session {
	...
	gboolean _bcheckingTS;	// add kimi
	uint32_t _saveTS;		// add kimi
	gint64	 _frameProcUs;	// add kimi
	gint64 	 _prevQueuingUs;	// add kimi
	gint64	 _firstQueuingUs;	// add kimi

	// add kimi for simulcast ABR
	simulcast_abr_state_context _simulcast_abr_ctx;
...
void janus_streaming_incoming_rtcp(janus_plugin_session *handle, int video, char *buf, int len) {
	...
	if(!video && (source->audio_rtcp_fd > -1) && (source->audio_rtcp_addr.ss_family != 0)) {
		...
		if(bw > 0) {
			...
			JANUS_LOG(LOG_INFO, "  -- REMB for this PeerConnection: %"SCNu64"\n", bw);
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(!strcasecmp(request_text, "watch")) {
			...
			if(mp->codecs.audio_pt >= 0 && session->audio) {
				...
				// add kimi
				if (mp->streaming_source == janus_streaming_source_rtp) {
					g_strlcat(sdptemp, "a=extmap:1 urn:ietf:params:rtp-hdrext:ssrc-audio-level\r\n", 2048);
				}				
			...
			if(mp->codecs.video_pt > 0 && session->video) {
				...
				// add kimi
				if (mp->streaming_source == janus_streaming_source_rtp) {
					g_strlcat(sdptemp, "a=extmap:13 urn:3gpp:video-orientation\r\n", 2048);
					g_strlcat(sdptemp, "a=extmap:12 http://www.webrtc.org/experiments/rtp-hdrext/playout-delay", 2048);
				}
				g_strlcat(sdptemp, "a=sendonly\r\n", 2048);
			}
#ifdef HAVE_SCTP
...
static void *janus_streaming_relay_thread(void *data) {
	...
	gint64 frameRecvDurationUs = 0;	// add kimi
	gint64 prevQueuingUs = 0;	// add kimi
	gint64 firstQueuingUs = 0;	// add kimi
		...
		for(i=0; i<num; i++) {
			if(fds[i].revents & (POLLERR | POLLHUP)) {
				...
				if(pipe_fd != -1 && fds[i].fd == pipe_fd) {
					...
					packet.origin_timestamp = ntohl(rtp->timestamp);		// add kimi
					packet.origin_seq_number = ntohs(rtp->seq_number);		// add kimi
...
static void janus_streaming_helper_rtprtcp_packet(gpointer data, gpointer user_data) {
	...
	copy->origin_timestamp = packet->origin_timestamp;		// add kimi
	copy->origin_seq_number = packet->origin_seq_number;	// add kimi	
	copy->queuing_nowUs = packet->queuing_nowUs;			// add kimi
...
static void *janus_streaming_helper_thread(void *data) {
	...
	pid_t tid = syscall(SYS_gettid); //gettid();	// add kimi

	gint64	 frameProcUs = 0;		// add kimi
	gint64 	 prevQueuingUs = 0;		// add kimi	
	gint64 	 queuingUs=0;				// add kimi
	gboolean bcheckingTS = FALSE;	// add kimi
	uint32_t saveTS=0;				// add kimi
...
/*
		...
		gint64 nowUs;
		if(pkt->is_video) {	
			if(bcheckingTS == FALSE) {															// add kimi
				bcheckingTS = TRUE;
				saveTS = pkt->origin_timestamp;
				prevQueuingUs = pkt->queuing_nowUs;
				queuingUs = pkt->queuing_nowUs;
			} else {
				if(saveTS != pkt->origin_timestamp) {
					nowUs = janus_get_monotonic_time();
					JANUS_LOG(LOG_INFO, "\tkimi_outgoing janus_streaming_helper_thread id %d thread id %d origin ts \t%d\t queue length \t%d\t dequeue procUs \t%ld \tqueuingUs \t%ld \tnowUs \t%ld\n", 
					helper->id, tid, saveTS, g_async_queue_length(helper->queued_packets), frameProcUs, queuingUs, nowUs);		// add kimi	
					...
					nowUs = janus_get_monotonic_time();
					frameProcUs += nowUs - prevQueuingUs;
					prevQueuingUs = nowUs;
				}
			}
		}	
