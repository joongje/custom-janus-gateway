...
#define __ENABLE_SUBSCRIBER_DATA_CHANNEL_WRITE__		// add kimi
...
// add kimi
static void janus_videoroom_incoming_data_from_subscriber(janus_plugin_session *handle, char *label, gboolean textdata, char *buf, int len);
...
static gboolean _bcheckingTS = FALSE;	// add kimi
static uint32_t _saveTS;		// add kimi
...
// add kimi
static void janus_videoroom_relay_data_packet_to_subscriber(janus_videoroom_subscriber *subscriber, janus_videoroom_rtp_relay_packet *packet);
static void janus_videoroom_relay_data_packet_to_publisher(janus_videoroom_publisher *publisher, janus_videoroom_rtp_relay_packet *packet);
...
// add kimi
static void janus_videoroom_subscriber_dereference_nodebug(janus_videoroom_subscriber *s) {
...
static guint32 janus_videoroom_rtp_forwarder_add_helper(janus_videoroom_publisher *p,
	...
	janus_mutex_unlock(&p->rtp_forwarders_mutex);
	JANUS_LOG(LOG_INFO, "Added %s/%d rtp_forward to participant %"SCNu64" host: %s:%d stream_id: %"SCNu32" %s\n",
		is_data ? "data" : (is_video ? "video" : "audio"), substream, p->user_id, host, port, stream_id, fd > -1 ? "rtcp enabled":"rtcp disabled");
	return stream_id;
}
...
static json_t *janus_videoroom_process_synchronous_request(janus_videoroom_session *session, json_t *message) {
	...
	if(!strcasecmp(request_text, "create")) {
		...
		JANUS_LOG(LOG_VERB, "rtp_forward++++\n");	// add kimi
		JANUS_LOG(LOG_VERB,"%s\n", json_dumps(message, 0));		// add kimi
		JANUS_LOG(LOG_VERB, "rtp_forward----\n");	// add kimi
...
/*
		if(video) {			// add kimi
			gboolean blog = FALSE;
			if(_bcheckingTS == FALSE) {															
				_bcheckingTS = TRUE;
				_saveTS = rtp->timestamp;
			} else {
				if(_saveTS != rtp->timestamp) {
					_saveTS = rtp->timestamp;
					blog = TRUE;
				}
			}
			...
			if(blog == TRUE) {
				JANUS_LOG(LOG_INFO, "kimi_incoming incoming_video_rtp frame  timestamp %u  seq %u   nowUs \t%ld\n",
				ntohl(rtp->timestamp), ntohs(rtp->seq_number), janus_get_monotonic_time());		// add kimi	
			}
		}
*/
...
//	JANUS_LOG(LOG_INFO,"janus_videoroom_incoming_rtcp\n");	
...
#ifdef __ENABLE_SUBSCRIBER_DATA_CHANNEL_WRITE__			// kimi
	if(!session || g_atomic_int_get(&session->destroyed)) 
		return;
	if(session->participant_type == janus_videoroom_p_type_subscriber) {
		janus_videoroom_incoming_data_from_subscriber(handle, label, textdata, buf, len);
		return;
	}	
#else
	if(!session || g_atomic_int_get(&session->destroyed) || session->participant_type != janus_videoroom_p_type_publisher)
...
static void janus_videoroom_relay_data_packet_to_subscriber(janus_videoroom_subscriber *subscriber, janus_videoroom_rtp_relay_packet *packet) {
	...
	if(gateway != NULL) {
		JANUS_LOG(LOG_VERB, "Subscriber Message, Forwarding %s DataChannel message (%d bytes) to viewer\n",
...
static void janus_videoroom_incoming_data_from_subscriber(janus_plugin_session *handle, char *label, gboolean textdata, char *buf, int len) {
	janus_videoroom_session *session = (janus_videoroom_session *)handle->plugin_handle;
	janus_videoroom_subscriber *participant = janus_videoroom_session_get_subscriber_nodebug(session);
	if(participant == NULL)
		return;
	
	if(g_atomic_int_get(&participant->destroyed) || participant->kicked) {
		janus_videoroom_subscriber_dereference_nodebug(participant);
		return;
	}
	
	JANUS_LOG(LOG_VERB, "Got a %s DataChannel(from subscriber) message (%d bytes) to forward\n",
	textdata ? "text" : "binary", len);
	...
	janus_videoroom_publisher *publisher = participant->feed;
	...
	GHashTableIter iter;
	gpointer value;
	g_hash_table_iter_init(&iter, publisher->rtp_forwarders);
	while(publisher->udp_sock > 0 && g_hash_table_iter_next(&iter, NULL, &value)) {
		janus_videoroom_rtp_forwarder* rtp_forward = (janus_videoroom_rtp_forwarder*)value;
		if(rtp_forward->is_data) {
			struct sockaddr *address = (rtp_forward->serv_addr.sin_family == AF_INET ?
				(struct sockaddr *)&rtp_forward->serv_addr : (struct sockaddr *)&rtp_forward->serv_addr6);
			size_t addrlen = (rtp_forward->serv_addr.sin_family == AF_INET ? sizeof(rtp_forward->serv_addr) : sizeof(rtp_forward->serv_addr6));
			if(sendto(publisher->udp_sock, buf, len, 0, address, addrlen) < 0) {
				JANUS_LOG(LOG_HUGE, "Error janus_videoroom_incoming_data_from_subscriber forwarding data packet for %s... %s (len=%d)...\n",
					publisher->display, strerror(errno), len);
			}
		}
	}
	janus_mutex_unlock(&publisher->rtp_forwarders_mutex);

	janus_mutex_lock_nodebug(&publisher->subscribers_mutex);
	GList *subscribes = (GList*)publisher->subscribers;
	while(subscribes) {
		if((janus_videoroom_subscriber*)subscribes->data != participant) {
			janus_videoroom_relay_data_packet_to_subscriber((janus_videoroom_subscriber*)subscribes->data, &packet);
		}
		subscribes = subscribes->next;
	}
	janus_mutex_unlock_nodebug(&publisher->subscribers_mutex);

	janus_videoroom_relay_data_packet_to_publisher(publisher, &packet);		// kimi think.... crash....
