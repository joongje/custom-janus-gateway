...
static json_t *janus_info(const char *transaction) {
	...
	if(janus_ice_get_turn_server() != NULL) {
		...
		JANUS_LOG(LOG_VERB, ">>>>>>>>>> turn-server  %s\n", server);
	}
...
void janus_transport_task(gpointer data, gpointer user_data) {
	...
	if(request == NULL) {
//		JANUS_LOG(LOG_ERR, "kimi___ Missing request\n");
		return;
	}

//	JANUS_LOG(LOG_VERB, "kimi___ janus_transport_task+++ admin %d\n", request->admin);	// add kimi
	if(!request->admin)
...
//	JANUS_LOG(LOG_VERB, "kimi___ janus_transport_task---\n");		// add kimi
}
