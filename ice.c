		...
		if(janus_is_webrtc_encryption_enabled() && (!component->dtls || !component->dtls->srtp_valid || !component->dtls->srtp_in)) {
			...
			if(res != srtp_err_status_ok) {
				...
				if(!janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_HAS_AUDIO)) {
					...
					JANUS_LOG(LOG_VERB, "[%"SCNu64"] Incoming RTCP, bundling: this is video (no audio has been negotiated)\n", handle->handle_id);	// kimi LOG_HUGE -> LOG_VERB
					video = 1;
					...
					JANUS_LOG(LOG_VERB, "[%"SCNu64"] Incoming RTCP, bundling: this is audio (no video has been negotiated)\n", handle->handle_id);	// kimi LOG_HUGE -> LOG_VERB
					video = 0;
				} else {
...
//						JANUS_LOG(LOG_HUGE, "__xxxx__ peer=0 (audio_ssrc_peer %u video_ssrc_peer %u) rtcp_ssrc %u audio_ssrc %u video_ssrc %u\n", 
//									stream->audio_ssrc_peer, stream->video_ssrc_peer[0], rtcp_ssrc, stream->audio_ssrc, stream->video_ssrc);	// kimi
						if(rtcp_ssrc == 0) {
...
//							JANUS_LOG(LOG_HUGE, "__xxxx__ [%"SCNu64"] janus_rtcp_has_fir || janus_rtcp_has_pli || janus_rtcp_get_remb unknown SSRC (%"SCNu32")\n", handle->handle_id, rtcp_ssrc);	// add kimi
							video = 1;
						} else {
...
//						JANUS_LOG(LOG_HUGE, "__xxxx__ rtcp_ssrc %u audio_ssrc %u video_ssrc %u\n", rtcp_ssrc, stream->audio_ssrc, stream->video_ssrc);	// kimi
						if(rtcp_ssrc == 0) {
							/* No SSRC, maybe an empty RR? */
							return;
						}
				...
				if (janus_rtcp_parse(rtcp_ctx, buf, buflen) < 0) {
					...
					JANUS_LOG(LOG_VERB, "__xxxx__ janus_rtcp_parse error!!!!\n");	// add kimi
					return;
				}

//				JANUS_LOG(LOG_VERB, "[%"SCNu64"] Got %s RTCP (%d bytes)\n", handle->handle_id, video ? "video" : "audio", buflen);
				...
				if(nacks_count && ((!video && component->do_audio_nacks) || (video && component->do_video_nacks))) {
					...
					while(list) {
						...
						if(p == NULL) {
							JANUS_LOG(LOG_HUGE, "[%"SCNu64"]   >> >> Can't retransmit packet %u, we don't have it...\n", handle->handle_id, seqnr);  // kimi LOG_HUGE -> LOG_VERB
						} else {
							/* Should we retransmit this packet? */
							if((p->last_retransmit > 0) && (now-p->last_retransmit < MAX_NACK_IGNORE)) {
								JANUS_LOG(LOG_VERB, "[%"SCNu64"]   >> >> Packet %u was retransmitted just %"SCNi64"ms ago, skipping\n", handle->handle_id, seqnr, now-p->last_retransmit); // kimi LOG_HUGE -> LOG_VERB
								list = list->next;
								continue;
							}
							in_rb = 1;
				...
				if(component->retransmit_recent_cnt &&
					...
					JANUS_LOG(LOG_VERB, "[%"SCNu64"] _trace_check_retransmitted_ Retransmitted %u packets due to NACK (%s stream #%d)\n",
						handle->handle_id, component->retransmit_recent_cnt, video ? "video" : "audio", vindex);	// kimi append
...
//							janus_rtp_header *header = (janus_rtp_header *)pkt->data;
//							guint16 seq = ntohs(header->seq_number);
//							JANUS_LOG(LOG_VERB, "nice_agent_send %s seq \t%u\t nowUs \t%"SCNu64"\n",
//							pkt->type == JANUS_ICE_PACKET_VIDEO ? "video" : "audio", seq, janus_get_monotonic_time());
//						}
//					}

					int sent = nice_agent_send(handle->agent, stream->stream_id, component->component_id, protected, pkt->data);
					if(sent < protected) {
						janus_rtp_header *header = (janus_rtp_header *)pkt->data;	// add kimi
						JANUS_LOG(LOG_ERR, "[%"SCNu64"] _trace_send_fail_ ... only sent %d bytes? (was %d) %s packet timestamp %d seq_num %d\n", 
						handle->handle_id, sent, protected, pkt->type == JANUS_ICE_PACKET_VIDEO ? "video" : "audio", ntohl(header->timestamp), ntohs(header->seq_number));	// kimi append why???
					}
					/* Update stats */
					if(sent > 0) {
						/* Update the RTCP context as well */
