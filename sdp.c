	...
	if(res >= 7) {
		...
		if(component == NULL || rcomponent > 1) {
			...
			if(c != NULL) {
				...
				if(trickle) {
					gsize bit = ((gsize) g_atomic_pointer_get(&handle->webrtc_flags));
					JANUS_LOG(LOG_VERB, "[%"SCNu64"] webrtc_flags 0x%02lX candidate >>> %s\n", handle->handle_id, bit, candidate);		// kimi
					if(janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_START)) {
						...
						if(!component->process_started) {
							...
							if(nice_agent_set_remote_candidates(handle->agent, stream->stream_id, component->component_id, candidates) < 1) {
								...
								JANUS_LOG(LOG_VERB, "[%"SCNu64"] Trickle candidate added!  JANUS_ICE_HANDLE_WEBRTC_START process_started\n", handle->handle_id);
							}
						...
						if(!janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_PROCESSING_OFFER)) {
							...
							if(!component->process_started) {
								...
								if(nice_agent_set_remote_candidates(handle->agent, stream->stream_id, component->component_id, candidates) < 1) {
									...
									JANUS_LOG(LOG_VERB, "[%"SCNu64"] Trickle candidate added!\n !JANUS_ICE_HANDLE_WEBRTC_PROCESSING_OFFER process_started", handle->handle_id);
								}
								g_slist_free(candidates);
							}
						} else {
